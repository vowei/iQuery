...
public class iQuery {
	public static <T extends ITreeNode> List<T> query(T root, String iquery) throws IOException, RecognitionException, iQueryParseException {
		iQueryParser parser = createParser(iquery);
		List<ITreeNode> candidates = new ArrayList<ITreeNode>();
		candidates.add(root);
		
		@SuppressWarnings("unchecked")
		List<T> result = (List<T>)parser.query(candidates);
		if ( parser.getErrors().size() > 0 ) {
			throw new iQueryParseException(parser.getErrors());
		} else {
			return result;
		}
	}
